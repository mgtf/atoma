// Compiled host path, production dependencies only, no provider calls or worker permissions.
...
try {
  ...
  const index = ProjectRetrievalIndex.open(store);
  ...
  console.log(`SQLite FTS5 compiled retrieval smoke passed (${process.version})`);
} finally {
  await tool?.close();
  closeStoreHandles();
