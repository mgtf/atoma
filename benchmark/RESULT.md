...
The initial diagnosis below — recipe competition would freeze the two
verification recipes forever — was later corrected by round 2: both did gain a
second match, and the real constraint was the task decomposition rarely
producing a standalone re-verification phase. The immutable round-1 observation
is retained here; the correction and subsequent rounds live in
[`ROUND2.md`](ROUND2.md) through [`ROUND8.md`](ROUND8.md).
...
- **One task family.** CLI-plus-documentation is where atoma's machinery is
  most developed — its best case, stated in advance.
- **Small n.** Five baseline and ten treatment runs. No significance test is
  claimed.
- **The instrument was corrected twice, both times in atoma's favour** — see
  the amendments in `PROTOCOL.md`. Both were verified false negatives and all
  seven control deliverables held full marks across revisions.

## Reproducing

```bash
ATOMA_LLM=claude-cli npm run benchmark -- --dry-run
ATOMA_LLM=claude-cli npm run benchmark
node benchmark/score-all.mjs
node benchmark/plot.mjs
```

Committed evidence: `results.csv`, `scores.json` and
[`traces.tar.gz`](traces.tar.gz). Runtime logs and `runs/` remain ignored.
