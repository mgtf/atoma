...
assert.equal(recordedRows.length, registration.schedule.length);
assert.deepEqual(pairedRetrievalDecision(registration, recordedRows), json(join(root, 'report.json')).comparison);
const rows = [];
...
assert.equal(rows.length, registration.schedule.length);
