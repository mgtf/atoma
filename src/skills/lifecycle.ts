...
export function skillContextBlock(skill: {
  ...
  if (skill.kind === 'script') {
    ...
    return [
      ...
      `     leftover ${filename} makes that check fail. rmSync's exit 0 IS`,
      `     the confirmation — do not spend another call reading the deletion`,
      `     back (no ls, no list_files, no read_file).`,
...
export class SkillLifecycle {
  ...
  async learnSkillFromRun(args: {
    ...
    const userContent = [
      ...
      `teach THAT.`,
      ``,
      `HARD RULE — "when_to_use" IS MATCHED AGAINST THE SUBTASK TEXT ALONE.`,
      `The matcher is a cheap model that sees the NEXT subtask's wording and`,
      ...
      `runs, while every sibling phrased as what the TASK ASKS FOR ("Task asks`,
      `to add package.json + README to an already-tested CLI", "README/task`,
      `lists concrete invocations with expected exit codes to check") drew 3 to`,
      `22 matches. Same catalog, same host atom, opposite outcomes.`,
      `So: phrase it as the REQUEST or the PHASE, never the file system. If`,
      `prior state genuinely matters, name the phase that produces it — "after`,
      `the CLI has been built and probed, re-check it still behaves as`,
      `recorded" is matchable; "a .atoma-probes.json exists" is not.`,
      ``,
      ...
      `                   See the HARD RULE below — this field decides whether the`,
      `                   recipe is ever picked at all.`,
      ...
      `actually produce.`,
      `ITS "when_to_use" IS WHERE THIS SPLIT USUALLY DIES. A verification recipe`,
      `acts on artefacts a previous phase produced, so it is the most tempting`,
      `one to describe as disk state — and that is exactly the unmatchable`,
      `phrasing the HARD RULE above forbids. Both measured casualties were`,
      `verification recipes. Phrase it as the request the next subtask will`,
      `make ("the task asks to run the documented invocations and confirm the`,
      `recorded exit codes still hold"), and say what it is NOT for, so it does`,
      `not compete with the build recipe for build phases.`,
