...
export function buildCompileSkillPrompt(args: {
  ...
  return [
      ...
      `PACKAGE SCRIPTS MUST BE PROVEN. Never synthesize package.json "start"`,
      `or "test" commands from the entry filename alone: a CLI that requires`,
      `arguments makes \`node ./entry.js\` fail by construction. Add a script`,
      `only when its exact command already succeeded in machine-recorded probes`,
      `and the required fixtures ship with the deliverable; otherwise omit the`,
      `scripts field. Observed failure: deterministic packaging produced`,
      `"test":"node ./dedupe-lines.js", so npm test always exited 1.`,
      ``,
      ...
      `  Promotable case: {"promotable": true, "language": "node", "body": "<full script source>", "writes": ["<workspace-relative path>", ...]}`,
      `  Refusal case:    {"promotable": false, "reason": "<one sentence>"}`,
      `"writes" declares every file the script CREATES or MODIFIES on a`,
      `successful run (the probe manifest included, when the script merges`,
      `observations into it). List concrete workspace-relative paths; for a`,
      `destination derived at runtime, declare the pattern's literal parts you`,
      `can guarantee, and omit what you cannot. Declare an empty array only`,
      `for a script that writes nothing. The runtime uses this list to decide`,
      `which subtasks the script may serve, so an over-claim wastes dispatches`,
      `and an under-claim permanently hides the script from valid work.`,
