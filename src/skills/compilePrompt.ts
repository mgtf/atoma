...
export function buildCompileSkillPrompt(args: {
  ...
  return [
      ...
      `INTERPRETER TOKENS ARE NOT PRODUCT NAMES. In a usage line such as`,
      `\`Usage: node index.js <file>\`, \`node\` is the interpreter and`,
      `\`index.js\` is a generic entry path — neither is an honest package, bin`,
      `or README title. Prefer existing package metadata or an unambiguous`,
      `semantic entry filename. If the only candidates are generic launch`,
      `syntax (\`node\`, \`index\`, \`main\`, \`cli\`), exit NON-ZERO and let`,
      `the validated LLM path derive the name from the task; never publish a`,
      `package literally named \`node\` or \`index\`.`,
      ``,
      `QUANTIFIER SCOPE IS SEMANTIC. If a requirement says a NAMED section`,
      `(\`## Steps\`) contains at least N bullets, apply N to that section`,
      `only — never to every sibling heading. Observed failure: a compiled`,
      `markdown verifier required three bullets in prose-only \`## When to use\``,
      `because it globalised the nearby Steps constraint, false-failed three`,
      `correct files, and forced the whole validated LLM fallback.`,
      ``,
