...
- HANDHELD DEVICES ARE GATED, temporarily (2026-09-18): the mobile journey is neither finished nor tested, so a phone never enters the product. `isHandheldDevice()` (`client-gl/handheld.ts`) is the ONE predicate — pointer capability, `(any-pointer: coarse) and (any-hover: none)`, never a viewport width or a user agent; `?atomaHandheld=1` rehearses it on a desktop — sampled into `store.handheld` and refreshed from the live media query. Behind it the arrival gate draws the hero crystal and ONE Continue whether or not the auth gate is on (provider buttons would start an OAuth flow into a product the phone cannot use); `enter()` refuses and flips `handheldBlocked` instead, the one signal the Pixi control and its DOM mirror both read to re-label and disable; and `useHandheldWhiteout` drives ONE rAF loop that swells the bead (`renderer/mark-surge.ts`, a mutable sample the mark's paint reads once per frame and publishes the bead's screen position from) while a white DOM flood grows from that position until the whole interface is white. Once white the scene host UNMOUNTS, so a phone does not keep rendering a hero crystal under a blank page. Reduced motion jumps to white. `viz:shot --handheld` is the on-device proof; remove the gate by deleting the predicate's consumers, never by narrowing the query.
