...
- The renderer draws in two PIXI spaces. `stage` is the persistent scene root;
  ...
  Overview is identity; nav zooms to the compact rail, re-activation restores overview. Routing to ANOTHER section at an unchanged mode turns the whole interface like a box (`cube-turn.ts`, `CubeTurnPlane`) and plays a half-amplitude navigation shot inside the face: one beat, one duration, both read from ONE route (`navigation-intent.ts`) — rail rows set amplitude, the rail GROUP boundary sets the axis (swing within a group, tip across one). There is no cube: only the leaving and arriving faces exist and they are rebuilt per route, so six faces never bound the rail. The turn is CSS 3D on a wrapper ABOVE the camera plane, never camera yaw — the camera stays face-on inside the face, so Pixi's affine transform stays the exact homography the overlays and hit tests use; the arriving face is the LIVE canvas plus live DOM in one transformed subtree (forms keep state), the leaving one a still from `captureSceneBitmap` (off-screen re-render + readback, resolution 1, released at settle) and therefore carries no DOM. Faces must be OPAQUE (`backgroundAlpha: 0` would show one through the other) and are ordered by depth, not `backface-visibility`, which cannot cull a wall parallel to the line of sight. Canvas pointer input is suspended while it turns because the inverse hit test knows the camera, not the box; DOM overlays stay live. The camera shot itself stays on its axis — the top-right pin keeps every edge covered and its landing overshoot may spend the focused crop's 8px rail guard and NO more, or the destination icon column clips. The light sweep shares the same duration. `SceneCameraPlane`
