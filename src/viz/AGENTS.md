...
- The renderer draws in two PIXI spaces. `stage` is the persistent scene root;
  ...
  Overview is identity; nav zooms to the compact rail, re-activation restores overview. Routing to ANOTHER section at an unchanged mode plays the navigation shot rather than swapping content under a static lens: the SAME overview↔focus axis, out and back in one beat, amplitude/duration from the rail rows the click travelled and resumed from the live axis so a second click continues instead of cutting. Staying on that axis is the point — face-on keeps Pixi's affine transform exact and the top-right pin keeps every edge covered; the landing overshoot may spend the focused crop's 8px rail guard and NO more, or the destination icon column clips. The light sweep shares its duration. `SceneCameraPlane`
