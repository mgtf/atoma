...
- The procedural far field (aurora, moving grid/scan and square stars) is HERO
  SCENERY only. Keep its compiled resources, but detach both its fullscreen
  mesh and ticker after entry unless the retained crystal is at
  `ATOMA_MARK_ENV_MIN_SCALE` or larger. Ordinary app views have no replacement
  ambient grid. Compact crystal chrome also publishes no far-field spills or
  caustics; one mark frame captures canvas/camera projection once and shares it
  across every hero spill/corner.
