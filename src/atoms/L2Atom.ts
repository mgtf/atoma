...
export class L2Atom extends Atom implements Supervisor<L1Atom>, Peerable<L2Atom> {
  ...
  async plan(task: Task, ctx: RunContext): Promise<Plan> {
    ...
    const userContent = [
      ...
      `shells, start servers, or validate anything. Your role is to shape the`,
      `task into one or more real responsibilities and route each to an L1`,
      `molecule (the only tier that can call tools).`,
      ``,
      `== DECOMPOSITION DISCIPLINE ==`,
      `Return a LIST of subtasks. Each subtask has ONE cohesive responsibility`,
      `and targets a specific L1 molecule via "preferredChild" (required for`,
      `N>1, optional for N=1 where your strategy still drives selection).`,
      `Independent responsibilities may run in parallel. Dependent phases use`,
      `sequential aggregation. Do not label dependent work orthogonal.`,
      `VERIFICATION IS A REQUIRED OUTCOME, NOT A DEFAULT PHASE. A cohesive,`,
      `non-destructive write → boot/serve → probe sequence belongs in one L1`,
      `subtask when the same tool bucket can execute every required`,
      `observation. N=1 is valid for that cohesive outcome even when the`,
      `artefact is a non-trivial app, game, library or service.`,
      `Use separate responsibilities when the task explicitly requires a`,
      `separate phase, observations cross executable tool buckets or require`,
      ...
      `at a distinct point. File count`,
      `alone does not erase a bucket boundary.`,
      ``,
      `== AGGREGATION ==`,
      ...
      `    only for a real dependency or a verification boundary defined above.`,
      `    The final phase's output is the deliverable; no extra LLM merge call.`,
      `    Do not create one solely to repeat probes the writer already passed.`,
      ``,
