...
export class L2Atom extends Atom implements Supervisor<L1Atom>, Peerable<L2Atom> {
  ...
  > {
    const userContent = [
      ...
      `[{"cmd", "exitCode", "stdout", "stderr"}]} — the record of invocations`,
      `already executed and verified by earlier phases. When the recipe`,
      `involves re-running / verifying / documenting invocations, the script`,
      `MUST read this manifest as its PRIMARY input (re-run each "cmd"`,
      `verbatim, compare observed exit/stdout/stderr byte-for-byte against the`,
      `recorded values) and fall back to prose parsing only when the manifest`,
      `is absent. Two compile generations of prose-parsing verification failed`,
      `offline regression on 6/6 real workspaces — free-form markdown is not a`,
      `parseable interface; the manifest is. Conversely, when the script`,
      `itself executes and verifies invocations, it MUST write/merge this`,
      `manifest so later passes inherit a machine-readable record.`,
      ``,
