...
export const SMOKE_DESIGN_GUIDANCE = [
  ...
  `\`ok === true\`. If you omit \`ok\`, any false boolean anywhere in the`,
  `object is treated as failure — an object containing`,
  `\`hasStreak3Class:false\` must never be accepted as proof of streak-3.`,
  ...
  `page throws before the smoke can run. Before finishing, scan the class for`,
  `duplicate method/getter names introduced during a fix; keep one definition.`,
  ...
  `inside a smoke IIFE, snapshot the milestone state, reset, snapshot again,`,
  `and return both objects.`,
  ``,
