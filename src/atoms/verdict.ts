...
export const VALIDATION_SYSTEM_PROMPT = [
  ...
  '    one cohesive responsibility, including a non-trivial app/game/library',
  '    write → boot/serve → probe flow when ONE executable tool bucket covers',
  '    every required observation. Verification is a required outcome, not a default phase.',
  '    DO NOT reject a plan just because N=1. DO NOT reject',
  ...
  '  - PARALLEL plans (mode "concat"|"llm-synthesize"): NO subtask depends on another\'s output.',
  '    Subtasks run via Promise.all; if the planner wrote',
  ...
  '    parallel) "concat" is almost always wrong — prefer "llm-synthesize".',
  '    Use "sequential" only for a real dependency: the task explicitly requires',
  ...
  '    a new claim needs fresh evidence; or volatile state must be observed at',
  '    a distinct point. Do NOT prefer sequential merely to repeat a probe',
  '    required after the responsible phase\'s final mutation.',
  '',
