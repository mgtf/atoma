...
export function manifestWriterLines(kind: 'shell' | 'http' | 'web'): string[] {
  if (kind === 'http') {
    return [
      ...
      `so record_probe refuses it. Boot through start_node_server, then call`,
      `fetch_url with "record": true for finite requests; only a finite test`,
      `harness belongs in a shell entry.`,
