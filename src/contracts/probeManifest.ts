...
export function manifestWriterLines(kind: 'shell' | 'http' | 'web'): string[] {
  if (kind === 'http') {
    return [
      ...
      `One entry per verified request, in the order you ran them. The file`,
      `ACCUMULATES across phases: READ it first if it exists and APPEND —`,
      `entries you did not write belong to earlier phases and must survive;`,
      `never truncate or rewrite it wholesale. APPEND in`,
      ...
      `so record_probe can only time it out and write a non-zero exit that proves`,
      `nothing. Boot it through the server lifecycle and record the finite HTTP`,
      `requests below; only a finite test harness belongs in a shell entry.`,
